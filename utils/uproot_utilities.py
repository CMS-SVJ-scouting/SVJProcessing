...
def __make_nano_aod_event_tree(ak_array, sort_by_name=True):
    ...
    # Counter branches (e.g. nJet, nFatJet) are auto-generated by uproot when
    # writing jagged collections — skip them here to avoid duplicate branches.
    counter_branches = {f"n{collection}" for collection in collections}

    # Fill in those dictionaries
    for field in ak_array.fields:
        if field in counter_branches:
            continue
